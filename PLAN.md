...
Dieses Dokument sammelt geplante Features. Status: `[ ]` offen, `[v]` erledigt, `[~]` in Arbeit.
...
### UI-Architektur: Inspector-Konzept
...
### Bookmarks
- [ ] Interessante Koordinaten mit Namen abspeichern und per Klick wieder anspringen
- [ ] Als Datei speichern/laden (nur Parameter: Koordinaten, Zoom, Iterationstiefe, Palettenreferenz — kein Bild, keine Map; muss beim Laden neu berechnet werden)
...
- [ ] Winkel einstellbar per Eingabefeld oder Slider
...
### Iterationstiefe
- [v] Maximale Iterationszahl konfigurierbar
...
- [v] Kurvenform einstellbar — LINEAR, SQRT, LOG in `PaletteMapper`
...
- [v] `IterationMap` ermittelt die kleinste tatsächlich vorkommende Escape-Iteration — einmalig am Ende von `tileIterate()`; auch in `FractalSnapshot` + `.mfrac` gespeichert
- [v] `PaletteMapper.configure()` normiert auf `[minIterations, maxIterations]` statt `[0, maxIterations]`
- Effekt: Bei tiefen Zooms wird die volle Palette auf den tatsächlichen Wertebereich gestreckt
...
### Zoom-Steuerung
- [v] Zoomfaktor statt roher Breite: Zoom 1 = volle Breite (~3), Zoom 10 = 1/10 davon, etc.
- [v] Ausschnittswahl per Maus (Rechteck aufziehen auf dem Fraktalbild)
...
- Design: Inspector zeigt immer das gerechnete Bild, nicht den aktuellen Viewport — visueller Zoom ist nur ein Vorschau-Trick
...
- Zukunft/Android: Visuelles D-Pad Widget als Alternative zu Pfeiltasten (relevant bei einer eventuellen Android-Portierung)

---

## Performance

### Swing-Threading-Architektur
...
- [v] `JFileChooser.setAccessory()` — kleines Preview-Panel neben dem Datei-Browser
- [v] Bei Auswahl einer `.mfrac`-Datei: Iterationsmap + Palette laden, skaliert als Vorschaubild anzeigen
- [v] `FractalIO.load()` im SwingWorker-Hintergrund; Generations-Counter verhindert veraltete Ergebnisse
- [v] Bei ungültiger Datei oder Ladefehler: Platzhaltertext
...
- [v] Alle UI-Texte über `ResourceBundle` externalisieren
