...
public class InspectorFrame extends JDialog {
    ...
    public InspectorFrame(RenderParameters params, Palette palette, ImageFrame imageFrame) {
    ...
    private JPanel buildColorPanel() {
        ...
        cb_palette.addActionListener(e -> {
            Object sel = cb_palette.getSelectedItem();
            if (sel != null) {
                log.debug("Palette gewechselt: {}", sel);
                palette.loadColors(PaletteLibrary.byName((String) sel));
            }
        });
        p.add(lbl_palette);
        p.add(cb_palette);
